...
class Helicopter:
    ...
    def RK45(self, x0, y0, ydot, h, u_input, trunc_error=False):
        ...
        k4 = h * np.array(ydot(y0 + self.B[3, 0] * k1 + self.B[3, 1] * k2 + self.B[3, 2] * k3, x0 + self.A[3] * h, *u_input), dtype=float)
        k5 = h * np.array(ydot(y0 + self.B[4, 0] * k1 + self.B[4, 1] * k2 + self.B[4, 2] * k3 + self.B[4, 3] * k4,
                  x0 + self.A[4] * h, *u_input), dtype=float)
        k6 = h * np.array(ydot(
            y0 + self.B[5, 0] * k1 + self.B[5, 1] * k2 + self.B[5, 2] * k3 + self.B[5, 3] * k4 + self.B[5, 4] * k5,
            x0 + self.A[5] * h, *u_input), dtype=float)
        y_new = y0 + k1 * self.CH[0] + k2 * self.CH[1] + k3 * self.CH[2] + k4 * self.CH[3] + k5 * self.CH[4] + k6 * self.CH[5]
        if trunc_error:
            trunc_error = k1 * self.CT[0] + k2 * self.CT[1] + k3 * self.CT[2] + k4 * self.CT[3] + k5 * self.CT[4] + k6 * self.CT[5]
        return y_new

    def RbI(self, THETA):
        A = sp.Matrix([[sp.cos(THETA[2]) * sp.cos(THETA[1]),
                        sp.cos(THETA[2]) * sp.sin(THETA[1]) * sp.sin(THETA[0]) - sp.sin(THETA[2]) * \
                        sp.cos(THETA[0]),
    ...
    def thetabi(self, THETA):
        A = sp.Matrix([[1, sp.sin(THETA[0]) * sp.tan(THETA[1]), sp.cos(THETA[0]) * sp.tan(THETA[1])],
    ...
    def Helicopter_model(self, t, x_state, U_input):
        u_velocity, v_velocity, w_velocity, p_angle, q_angle, r_angle, fi_angle, theta_angle, si_angle, xI, yI, zI, a_flapping, b_flapping, c_flapping, d_flapping = \
            x_state[0], x_state[1], x_state[2], x_state[3], x_state[4], x_state[5], x_state[6], x_state[7], x_state[8], \
            x_state[9], x_state[10], x_state[11], x_state[12], x_state[13], x_state[14], x_state[15]
        ...
        Kv = -Ku
        Kw = 16 * self.K_mu * mumr ** 2 * sp.sign(mumr) / (
        ...
        Xfus, Yfus, Zfus = -0.5 * self.rho * self.Sxfus * Vfus * (
                u_velocity - self.uwind), -0.5 * self.rho * self.Syfus * Vfus * (v_velocity - self.vwind), \
                           -0.5 * self.rho * self.Szfus * Vfus * (w_velocity - self.wwind - Vi)
        Fdrag = sp.Matrix([Xfus, Yfus, Zfus]).T
        mux, muy, muz = -(Uf[0]) / (self.OMEGA * self.Rmr), -(Uf[1]) / (self.OMEGA * self.Rmr), -(Uf[2]) / (
                self.OMEGA * self.Rmr)
        ...
        bTmr = 1 / 4 * self.rho * 3.1415 * self.Rmr ** 4 * self.OMEGA ** 2 * self.sigmamr * self.CLa * sp.Matrix(
            [mux ** 2 + muy ** 2 + 2 / 3, -muy, mux, 0])
        fQmr = 1 / 8 * self.rho * 3.1415 * self.Rmr ** 5 * self.OMEGA ** 2 * self.sigmamr * self.CLa * (
                self.CD0 / self.CLa * (1 + mux ** 2 + muy ** 2) - 2 * (muz - lambda0) ** 2)
        bQmr = 1 / 12 * self.rho * self.Rmr ** 2 * self.sigmamr * 3.1415 * self.CLa * \
               sp.Matrix([-self.Rmr ** 2 * (p_angle * (u_velocity - self.uwind) + q_angle * (
                       v_velocity - self.vwind) - 2 * romega * Vzi),
                          0.25 * self.Rmr * (6 * Vyp * Vzi - 3 * self.Rmr ** 2 * q_angle * romega),
                          -0.25 * self.Rmr * (6 * Vxq * Vzi - 3 * self.Rmr ** 2 * p_angle * romega), 0])
        fTtr = 1.5 * 1 / 12 * self.rho * self.Rtr ** 2 * self.sigmatr * 3.1415 * self.CLatr * self.Rtr * (
                ...
                - 2 * qomega * Vyi + (u_velocity - self.uwind) * p_angle + (
                        w_velocity - self.wwind - self.Kt * Vi) * r_angle)
        bTtr = 1 / 12 * self.rho * self.Rtr ** 2 * self.sigmatr * 3.1415 * self.CLatr * sp.Matrix([0, 0, 0, 3 *((
                    u_velocity - self.uwind) + q_angle * self.zfus - r_angle * self.yfus) ** 2 + 3 * (( w_velocity - 
                    self.wwind - self.Kt * Vi) + p_angle * self.yfus - q_angle * self.xfus) ** 2 + 2 * self.Rtr ** 2 * ( 
                        q_angle + self.OMEGAtr) ** 2])
        ...
        x_dot13 = -q_angle - a_flapping / self.taufb + 1 / (self.taufb * self.OMEGA * self.Rmr) \
                  * (Ku * (u_velocity - self.uwind) + Kw * (w_velocity - self.wwind)) + self.Alon / self.taufb * (
                          U_input[2] + Kc * c_flapping) - A_b * b_flapping / self.taufb
        x_dot14 = -p_angle - b_flapping / self.taufb + 1 / (self.taufb * self.OMEGA * self.Rmr) \
                  * Kv * (v_velocity - self.vwind) + self.Blat / self.taufb * (
                          U_input[1] + Kd * d_flapping) + B_a * a_flapping / self.taufb
        x_dot15 = -q_angle - c_flapping / taus + Clon / taus * U_input[2]
        x_dot16 = -p_angle - d_flapping / taus + Dlat / taus * U_input[1]
        return [x_dot1_3[0], x_dot1_3[1], x_dot1_3[2], x_dot4_6[0], x_dot4_6[1], x_dot4_6[2],
                x_dot7_9[0], x_dot7_9[1], x_dot7_9[2], x_dot10_12[0], x_dot10_12[1], x_dot10_12 \
                    [2], x_dot13, x_dot14, x_dot15, x_dot16]
